use hes_engine::kinds::{ByproductMap, Output, OutputMap, Resource, ResourceMap};
use leptos::*;
...
pub fn water_use_percent(l: f32) -> f32 {
    state!(|state, ui| {
        let total_water = state.resources.water;
        l / total_water * 100.
    })()
}
...
pub fn format_impact(impact: Impact, val: f32) -> String {
    match impact {
        ...
        Impact::Water => percent(water_use_percent(val) / 100., true),

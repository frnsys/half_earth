...
fn compute_tgav(
    ...
    nbox.get_mut("ffi_emissions").unwrap().splice(idx.., ffi_arr);
    nbox.get_mut("CH4_emissions").unwrap().splice(idx.., ch4_arr);
    nbox.get_mut("N2O_emissions").unwrap().splice(idx.., n2o_arr);
...
pub async fn calc_tgav(year: usize, annual_emissions: Vec<(f64, f64, f64)>) -> Result<f32, ServerFnError> {
    Ok(compute_tgav(year, &annual_emissions) as f32)
}

pub static STARTING_SURFACE: LazyLock<EarthSurface> = LazyLock::new(|| {
    // A grayscale image where each value
    // indicates the label of that pixel
    let label_data = include_bytes!("../../public/assets/surface/landuse.png");

    // A grayscale image that maps
    // temp (x-axis) and precip (y-axis)
    // to a biome label.
    let lookup_data = include_bytes!("../../public/assets/surface/biomes.png");
    ...
    let biome_labels: Vec<_> = biome_labels.buffer.as_bytes().iter().step_by(4).copied().collect();
    let biome_lookup: Vec<_> = biome_lookup.buffer.as_bytes().iter().step_by(4).copied().collect();

    let scale = 4;
    EarthSurface::new(biome_labels, width, height, scale, biome_lookup)
});

use crate::{write_state, state, views::{cards::{Draggable, Cards, CardFocusArea, DragRect}}, util::{to_ws_el, detect_center_element, nodelist_to_elements}, anim::animation};
use leptos::*;
...
pub fn Scanner(
    ...
    let (scan_anim, set_scan_anim) = create_signal(None);
    ...
    let scan_card = move || {
        if let Some(progress) = progress_ref.get() {
            let duration = scan_time * 1000. * scan_time_multiplier.get();
            let (_, stop, vals) = animation([0.], [100.], duration, move || {
                if is_scanning.get() {
                    set_scan_time_multiplier.update(|mult| {
                        *mult = (*mult * 4./5.).max(0.2);
                    });
                    on_finish_scan.call(());
                }
            }, true);
            set_scan_anim.set(Some(stop));
            create_effect(move |_| {
                if let Some(progress) = progress_ref.get() {
                    let p = vals.get()[0];
                    let width = format!("{p}%");
    ...
    let stop_scanning_card = move || {
        set_scan_time_multiplier.set(1.);
...
pub fn RemoveScanner(
    ...
    #[prop(into)] drag_rect: Signal<Option<DragRect>>,
    #[prop(into)] on_finish_scan: Callback<()>,
    #[prop(into)] set_y_bound: Callback<(f32, f32)>) -> impl IntoView {
...
pub fn ScannerCards<I: Scannable>(
    ...
    // let add_scanner_ref = create_node_ref::<AddScanner>();
    // let rem_scanner_ref = create_node_ref::<RemoveScanner>();
      ...
      // TODO use refs?
        if let Some(scroller) = document().query_selector(".cards").unwrap() {
            ...
            if els.length() == 0 {
                ...
                if let Some(idx) = detect_center_element(scroller.dyn_into::<web_sys::HtmlElement>().expect("Is an html element"), &els) {
    ...
    let y_bounds = move || {
        [top_y_bound.get(), bot_y_bound.get()]
    };
    ...
    view! {
        <Show when={move || focused().is_some()}>
            <AddScanner scan_time=add_props.scan_time
            drag_rect
        ...
        >
            <For each=move || items.get().into_iter().enumerate()
                ...
                children=move |(i, item)| {
                    let draggable = move || allow_swipe.get() && focused_idx.get() == Some(i);

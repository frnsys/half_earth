...
pub use describe_effects::EffectThing;
use hes_engine::{
    events::*,
    kinds::{Byproduct, Feedstock, Output, OutputMap, Resource},
    projects::{Project, Status, Type},
    state::State,
};
use leptos::{
    component, expect_context, view, with, For, IntoSignal, IntoView, MaybeSignal, RwSignal,
    Signal, SignalWith,
};
...
fn outcome_effects(project: &Project) -> Vec<EffectThing> {
    struct Count {
        effect: EffectThing,
        count: usize,
        hashes: HashSet<Vec<u8>>,
    }
    let mut all_effects: HashMap<String, Count> = HashMap::default();
    let n_outcomes = project.outcomes.len();
    for outcome in &project.outcomes {
        for effect in &outcome.effects {
            // TODO effect.is_hidden
            let key = effect.fingerprint();
            let mut efx = EffectThing::from(effect);
...
impl From<&Effect> for EffectThing {
...
#[component]
pub fn Effects(
    #[prop(into)] effects: Signal<Vec<EffectThing>>,
    #[prop(optional)] class: &'static str,
) -> impl IntoView {
    let items = state_with!(|state, ui, effects| {
        effects
            .iter()
            ...
            <For
                ...
                children=move |item| {
                    view! {
                        <HasTip tip=item.tip>
                            <div class="effect">
                                <div class="effect--text" inner-html=item.text></div>
                            </div>
                        </HasTip>
                    }
                }
...
mod describe_effects {
    ...
    fn icon_card_tag(name: &str, icon: &str) -> String {
        format!(r#"<div class="card-tag"><img src="{icon}">{name}</div>"#)
    }
    ...
    pub fn flag_tip(flag: Flag, demand: &OutputMap) -> Tip {
        match flag {
            ...
            Flag::Vegan => {
                let changed_demand = (demand.animal_calories * 0.9).round();
                tip! {
            ...
            Flag::Vegetarian => {
                let changed_demand = (demand.animal_calories * 0.75).round();
                tip! {
                    icons::PLANT_CALORIES,
                    r#"Animal calorie demand will change from <img src="{iconACals}">{prevDemandACals} to <img src="{iconACals}">{nextDemandACals} and plant calorie demand will change from <img src="{iconPCals}">{prevDemandPCals} to <img src="{iconPCals}">{nextDemandPCals}."#,
                    iconACals: icons::ANIMAL_CALORIES,
    ...
    impl EffectThing {
        ...
        fn change_dir(&self, change: f32) -> String {
            if self.is_unknown {
                ...
                let term = if change < 0. { "reduce" } else { "increase" };
                t!(&format!("{prob} {term}"))
            } else {
        ...
        pub fn tip(&self, state: &State) -> Result<EffectTip, ()> {
            let (tip, text) = match &self.effect {
                ...
                Effect::Resource(resource, amount) => {
                    let fmtted = format::resource(*amount, *resource).abs();
                    let percent = format::percent(amount / state.resources[*resource], true);
                    (
                        tip! {
                            resource.icon(),
                ...
                Effect::Output(output, amount) => {
                    let base = format::output(*amount, *output);
                    ...
                    (
                        ...
                        text! {
                            ...
                            percent: format::percent(amount.abs(), true),
                            name: t!(output.lower()),
                            changeDir: self.change_dir(*amount),
                            icon: output.as_key(),
                        },
                ...
                Effect::OutputForProcess(id, amount) => {
                    ...
                    (tip! {
                        ...
                        percent: format::percent(*amount, true),
                ...
                Effect::Demand(output, amount) => {
                    let demand = format::outputs(&state.output_demand);
                ...
                Effect::DemandAmount(output, amount) => {
                    let demand = format::outputs(&state.output_demand);
                    let amount = format::output(*amount, *output);
                    let current_demand = demand[*output];
                    let after_demand = demand[*output] + amount;
                    let demand_change =
                        (after_demand - current_demand) / state.world.demand()[*output];
                    (
                        tip! {
                            output.icon(),
                            ...
                            percent: format::percent(demand_change.abs(), true),
                            afterDemand: after_demand,
                            currentDemand: current_demand,
                            icon: output.icon(),
                            name: t!(output.lower()),
                ...
                Effect::UnlocksProject(id) => {
                    ...
                    let text = if self.is_unknown
                        && let Some(prob) = self.likelihood
                    {
                        t!(&format!("{prob} unlock the {{tag}} project."))
                    } else {
                ...
                Effect::UnlocksProcess(id) => {
                    ...
                    let text = if self.is_unknown
                        && let Some(prob) = self.likelihood
                    {
                        t!(&format!("{prob} unlock the {{tag}} process."))
                    } else {
                ...
                Effect::ProjectCostModifier(id, amount) => {
                    ...
                    let kind = match project.kind {
                        ...
                        Type::Initiative => t!("development time"),
                    };
                    ...
                    let tip_icon = match project.kind {
                        Type::Policy => format!(r#"<img src="{}" />"#, icons::POLITICAL_CAPITAL),
                        _ => "".into(),
                    };
                    ...
                    (
                        ...
                        text! {
                            ...
                            amount: if self.is_unknown {
                                ...
                                format::percent(amount.abs(), true)
                            },
                            changeDir: self.change_dir(*amount),
                            tag: tag,
                        },
                ...
                Effect::ModifyIndustryResources(id, resource, amount) => {
                    ...
                    let current_demand = format::resource(
                        industry.resources[*resource] * industry.demand(lic_pop),
                        *resource,
                    );
                    let after_demand = current_demand * (1. + amount);
                    let demand_change =
                        (after_demand - current_demand) / state.resources_demand[*resource];
                    let tag = card_tag(&t!(&industry.name));
                    let tip = if self.is_unknown {
                        ...
                        tip! {
                            ...
                            resource: t!(resource.lower()),
                            icon: resource.icon(),
                            percent: format::percent(demand_change, true),
                ...
                Effect::ModifyIndustryResourcesAmount(id, resource, amount) => {
                    ...
                    let current_demand =
                        format::resource(industry.resources[*resource] * demand, *resource);
                    let after_demand = format::resource(
                ...
                Effect::ModifyIndustryByproducts(id, byproduct, amount) => {
                    ...
                    let lic_pop = state.world.lic_population();
                    let demand = industry.demand(lic_pop);
                    let current = industry.byproducts.gtco2eq() * demand;
                    let after = current * (1. + amount);
                    let change = (after - current) / state.emissions_gt();
                    let tag = card_tag(&t!(&industry.name));
                    let tip_text = if self.is_unknown {
                        ...
                        emissionsChange: format::percent(change, true),
                ...
                Effect::ModifyProcessByproducts(id, byproduct, amount) => {
                    ...
                    let label = match byproduct {
                        ...
                        Byproduct::Co2 => t!("{type} emissions", type: t!("CO2")),
                        Byproduct::N2o => t!("{type} emissions", type: t!("N2O")),
                        Byproduct::Ch4 => t!("{type} emissions", type: t!("CH4")),
                    };
                    ...
                    let tip_text = if self.is_unknown {
                        ...
                        let change = match byproduct {
                            ...
                            _ => {
                                let current =
                                    process.byproducts.gtco2eq() * state.produced_by_process[*id];
                                let after = current * (1. + amount);
                                let change = (after - current) / state.emissions_gt();
                ...
                Effect::AddFlag(flag) => {
                    ...
                    let text = format!("<strong>{}</strong>", t!(&flag.to_string()));
                    (tip, text)
                }
                Effect::ProtectLand(amount) => (
                    ...
                    text! {
                        ...
                        percent: format::percent(amount/100., true),
                    },
                ),
                Effect::Feedstock(feedstock, amount) => {
                    let estimate = match feedstock {
                        ...
                        other => {
                            let est = state.feedstocks[*other] / state.consumed_feedstocks[*other];
                            Some(est.round())
                        }
                    };
                    let text = match estimate {
                        None => t!("We aren't tracking this feedstock."),
                        Some(0.) => t!("This feedstock has been depleted."),
                        Some(est) => {
                            if est.is_infinite() {

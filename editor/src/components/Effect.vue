...
<div class="effect">
  <fieldset>
    ...
    <div v-if="spec.choices">
      <label>Subtype</label>
      <select v-model="localData.subtype" @change="update">
        <option v-for="choice in spec.choices" :value="choice">{{choice}}</option>
      </select>
    </div>
    <div v-if="spec.entity">
      <label>Target</label>
      <select :id="`${localData.id}-entity-select`" v-model="localData.entity" @change="update">
    ...
    <div>
      <div v-if="spec.params" v-for="name in Object.keys(spec.params)">
        <template v-if="spec.params[name] == Number">
          <label>{{name}}</label>
          <input type="number" :value="localData.params[name]" @change="(ev) => updateParam(name, ev.target.value)" />
        </template>
      </div>
    </div>
  </fieldset>

...
fn project_view(
    ...
    egui::Frame::NONE
        .show(ui, |ui| {
            ui.add(inputs::edit(&mut project.name));
            ui.add(
            ...
            ui.add(inputs::toggle_enum(&mut project.kind).label("Type").help("The type of project."));
            ui.add(inputs::edit(&mut project.group).label("Category").help("The project's category."));
            ui.add(inputs::toggle(&mut project.ongoing, "Ongoing", "One-Shot").label("Upkeep").help("Is this a one-and-done project, or does it need continued maintenance?"));
            if project.kind == ProjectType::Initiative {
                ui.add(inputs::toggle(&mut project.gradual, "Gradual", "On Completion").label("Activation").help("Does this project have to be 100% finished before the effects occur, or do they develop as the project is developed?"));
            }
            ...
            match &mut project.base_cost {
                Cost::Fixed(cost) => {
                    ...
                    ui.add(inputs::edit(cost).label(label).help("A fixed project cost."));
                },
                Cost::Dynamic(mult, factor) => {
                    ui.add(inputs::edit(factor).label("Factor").help("The factor to use for computing the cost."));
                    ui.add(inputs::edit(mult).label("Factor Multiplier").help("The project's cost equals this value multiplied by the factor's value."));
                },
            }
            ...
            ui.add(inputs::toggle_enum(&mut cost_kind).label("Cost Kind").help("If this project uses a dynamically-calculated cost."));
            if cost_kind != prev {
                ui.memory_mut(|mem| {

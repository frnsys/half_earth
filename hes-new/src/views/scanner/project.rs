...
use crate::{
    consts,
    state::{PlanChange, StateExt, Tutorial},
    views::{
        CardState,
        cards::CardStateMut,
        scanner::ScanResult,
...
impl Scannable for Project {
    ...
    fn add_scan_done(
        ...
        let mut result = ScanResult::NoOp;
        let changes =
            ctx.plan_changes.entry(self.id).or_default();
        ...
        // Adding points to Research/Infrastructure
            ...
            && ctx.state.buy_point(&self.id, ctx.points)
        {
            if *ctx.tutorial == Tutorial::Projects {
                ctx.tutorial.advance();
            }
            ctx.state.assign_point(&self.id, ctx.points);
            ...
            && (changes.withdrawn
                || ctx.state.pay_points(&self.id))
        {
            if *ctx.tutorial == Tutorial::Projects {
                ctx.tutorial.advance();
            }
            ctx.state.pass_policy(&self.id);
        ...
        // TODO handle outside in scanner
        // if changed {
    ...
    fn rem_label(&self, ctx: &CardState) -> Option<String> {
        let label = if is_refundable(self, ctx.plan_changes) {
            t!("Undo")
    ...
    fn rem_scan_done(
        ...
        let refundable = is_refundable(self, ctx.plan_changes);
        let changes =
            ctx.plan_changes.entry(self.id).or_default();
        if refundable {
            if changes.upgrades > 0 {
                ctx.state.downgrade_project_x(
                    &self.id,
                    ctx.queued_upgrades,
                );
                changes.upgrades -= 1;
                ...
                ctx.state.stop_policy(&self.id);
                changes.passed = false;
            } else {
                let points = changes.points;
                let mut refund =
                    ctx.state.next_point_cost(&self.kind)
                        * points;
                ...
                if self.kind == ProjectType::Research {
                    let excess_points = points.saturating_sub(
                        ctx.points.refundable_research,
                    );
                    refund =
                        ctx.state.next_point_cost(&self.kind)
                            * (points - excess_points);
                    ctx.points.refundable_research = ctx
                        .points
                        .refundable_research
                ...
                ctx.state.unassign_points(&self.id, points);
                ctx.state
                    .change_political_capital(refund as isize);
                changes.points = 0;
            }
        } else if self.can_downgrade() {
            ctx.state.downgrade_project(&self.id);
            keep_withdrawing = self.level > 0;
            changes.downgrades += 1;
        } else {
            ctx.state.stop_project(&self.id);
    ...
    fn is_rem_visible(&self, ctx: &CardState) -> bool {
        self.is_rem_allowed(ctx)
    }

    fn is_rem_allowed(&self, ctx: &CardState) -> bool {
        is_subtractable(self, ctx.plan_changes)
    }
}

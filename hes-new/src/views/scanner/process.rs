use std::collections::BTreeMap;

use enum_map::EnumMap;
use hes_engine::{Id, Output, Process, State};
use rust_i18n::t;

use crate::{
    consts,
    state::Tutorial,
    views::scanner::ScanResult,
};

use super::Scannable;

fn is_subtractable(
    process: &Process,
    mix_changes: &EnumMap<Output, BTreeMap<Id, isize>>,
) -> bool {
    let change = mix_changes[process.output]
        .get(&process.id)
        .unwrap_or(&0);
    process.mix_share as isize + *change != 0
}

fn is_addable(
    process: &Process,
    state: &State,
    process_mix_changes: &EnumMap<Output, BTreeMap<Id, isize>>,
    points: isize,
) -> bool {
    let max_share = state.process_max_share(&process.id);
    let change = process_mix_changes[process.output]
        .get(&process.id)
        .unwrap_or(&0);
    points != 0 && *change < max_share as isize
}

fn remove_point(
    points: &mut isize,
    process: &Process,
    mix_changes: &mut EnumMap<Output, BTreeMap<Id, isize>>,
) {
    let change = mix_changes[process.output]
        .entry(process.id)
        .or_default();
    if process.mix_share as isize + *change > 0 {
        *points += 1;
        *change -= 1;
    }
}

// Returns the point change.
fn add_point(
    points: &mut isize,
    process: &Process,
    max_share: usize,
    mix_changes: &mut EnumMap<Output, BTreeMap<Id, isize>>,
) {
    if *points > 0 {
        let change = mix_changes[process.output]
            .entry(process.id)
            .or_default();
        if *change + 1 <= max_share as isize {
            *points -= 1;
            *change += 1;
        }
    }
}

impl Scannable for Process {
    fn add_label(
        &self,
        ctx: &crate::views::CardState,
    ) -> Option<String> {
        None
    }

    fn add_scan_time(&self) -> f32 {
        consts::PROCESS_CARD_SCAN_TIME
    }

    fn add_scan_done(
        &self,
        ctx: &mut crate::views::cards::CardStateMut,
    ) -> ScanResult {
        let addable = is_addable(
            self,
            ctx.state,
            ctx.process_mix_changes,
            *ctx.process_points,
        );
        if addable {
            if *ctx.tutorial == Tutorial::Processes {
                ctx.tutorial.advance();
            }

            let max_share =
                ctx.state.process_max_share(&self.id);

            add_point(
                ctx.process_points,
                self,
                max_share,
                ctx.process_mix_changes,
            );

            // TODO
            // Consider the process mix 'changed'
            // when all points have been assigned
            // if ctx.process_points == 0 {
            //     on_change.call(());
            // }

            if *ctx.process_points > 0 {
                ScanResult::SuccessContinue
            } else {
                ScanResult::SuccessStop
            }
        } else {
            ScanResult::Rejected
        }
    }

    fn is_add_visible(
        &self,
        ctx: &crate::views::CardState,
    ) -> bool {
        self.is_add_allowed(ctx)
    }

    fn is_add_allowed(
        &self,
        ctx: &crate::views::CardState,
    ) -> bool {
        is_addable(
            self,
            ctx.state,
            ctx.process_mix_changes,
            *ctx.process_points,
        )
    }

    fn rem_label(
        &self,
        ctx: &crate::views::CardState,
    ) -> Option<String> {
        Some(t!("Remove points").to_string())
    }

    fn rem_scan_time(&self) -> f32 {
        consts::PROCESS_CARD_WITHDRAW_TIME
    }

    fn rem_scan_done(
        &self,
        ctx: &mut crate::views::cards::CardStateMut,
    ) -> ScanResult {
        remove_point(
            ctx.process_points,
            self,
            ctx.process_mix_changes,
        );

        // If still subtractable, continue scanning
        if is_subtractable(self, ctx.process_mix_changes) {
            ScanResult::SuccessContinue
        } else {
            ScanResult::SuccessStop
        }
    }

    fn is_rem_visible(
        &self,
        ctx: &crate::views::CardState,
    ) -> bool {
        self.is_rem_allowed(ctx)
    }

    fn is_rem_allowed(
        &self,
        ctx: &crate::views::CardState,
    ) -> bool {
        is_subtractable(self, ctx.process_mix_changes)
    }
}

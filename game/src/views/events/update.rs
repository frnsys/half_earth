...
impl AsEventView for EngineUpdate {
    fn details<'a>(
        ...
        let (image, attrib) =
            image_for_update(self, &state.world);
    ...
    fn dialogue(
        &self,
        state: &State,
    ) -> Option<super::dialogue::Dialogue> {
        match self {
            EngineUpdate::Policy { id }
            | EngineUpdate::Project { id } => {
                let proj = &state.world.projects[id];
...
fn image_for_update<'a>(
    ...
    match update {
        ...
        EngineUpdate::Region { id, .. } => {
            ...
            let attrib =
                region.flavor.image.attribution.as_str();
            (flavor_image(&region.flavor.image), attrib)
        }
    }
}

fn render_project_outcomes(
    ui: &mut egui::Ui,
    proj: &Project,
    state: &State,
) {
...
fn render_region_outcomes(
    ...
    for (output, demand) in demand.items() {
        ...
        let prev_region_per_capita_demand =
            prev_demand[output] / pop;
        let prev_intensity = intensity::output_intensity(
            prev_region_per_capita_demand,
            output,
        );

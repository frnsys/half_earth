// Ported from Blender's Noise Texture shader node:
...
// noise is unsigned perlin noise, in (0, 1)
...
float noise_texture(vec3 co, float detail, float roughness, float distortion) {
  vec3 p = co;
...
float blender_noise(vec3 p, float scale, float detail, float roughness, float distortion) {
    return noise_texture(p * scale, detail, roughness, distortion);
}

...
> {
    let is_supported =
        use_supported(browser_supports_notifications);

    let (notification, set_notification) =
        create_signal(None::<web_sys::Notification>);

    let (permission, set_permission) =
        create_signal(NotificationPermission::default());
...
impl From<NotificationDirection>
    for web_sys::NotificationDirection
{
...
impl From<&UseWebNotificationOptions>
    for web_sys::NotificationOptions
{
...
#[cfg(not(feature = "ssr"))]
impl ShowOptions {
    fn override_notification_options(
        &self,
        options: &mut web_sys::NotificationOptions,
    ) {
        if let Some(direction) = self.direction {
            options.dir(direction.into());
        }

        if let Some(require_interaction) =
...
impl From<web_sys::NotificationPermission>
    for NotificationPermission
{
    fn from(
...
#[cfg(not(feature = "ssr"))]
async fn request_web_notification_permission(

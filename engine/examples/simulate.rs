use half_earth_engine::{
    byproducts, consts, content,
    events::Phase,
    game::{Difficulty, Game},
    kinds::{ByproductMap, Output, OutputMap, Resource, ResourceMap},
    production::{calculate_required, Process, ProcessFeature, ProductionOrder},
    projects::Status,
    resources,
};
            ...
            Scenario::Nuclear => {
                ...
                for process in &mut game.state.processes {
                    let is_nuclear = process.features.contains(&ProcessFeature::CanMeltdown)
                        || process
                            .features
                            .contains(&ProcessFeature::MakesNuclearWaste);
                    if is_nuclear {
...
impl CalibrationData {
    pub fn new(game: &Game, path: String) -> CalibrationData {
        ...
        cols.extend(
            game.state
                .processes
                .iter()
                .map(|p| format!("{:?}:{}:Mix Share", p.output, p.name)),
        );
        cols.extend(
            game.state
                .processes
                .iter()
                .map(|p| format!("{:?}:{}:Land Use", p.output, p.name)),
        );
        cols.extend(
            game.state
                .processes
                .iter()
                .map(|p| format!("{:?}:{}:Fuel Use", p.output, p.name)),
        );
        cols.extend(
            game.state
                .processes
                .iter()
                .map(|p| format!("{:?}:{}:Electricity Use", p.output, p.name)),
        );
        cols.extend(
            game.state
                .processes
                .iter()
                .map(|p| format!("{:?}:{}:CO2 Emissions (Gt)", p.output, p.name)),
        );
        cols.extend(
            game.state
                .processes
                .iter()
                .map(|p| format!("{:?}:{}:CH4 Emissions (Gt)", p.output, p.name)),
        );
        cols.extend(
            game.state
                .processes
                .iter()
                .map(|p| format!("{:?}:{}:N2O Emissions (Gt)", p.output, p.name)),
        );
        cols.extend(
            game.state
                .world
        ...
        cols.extend(
            game.state
                ...
                .keys()
                .iter()
                .map(|k| format!("Feedstock:{}", format!("{:?}", k))),
        );
    ...
    pub fn snapshot(&mut self, game: &Game, events: &Vec<(String, Option<String>)>) {
        ...
        let ind_demand = game
            ...
            .industries
            .iter()
            .fold(resources!(), |acc, ind| acc + ind.resources)
            * lic_pop;
        let industry_byproducts = game
            .state
            .industries
            .iter()
            .fold(byproducts!(), |acc, ind| acc + ind.byproducts)
            * lic_pop;

        let energy_orders: Vec<ProductionOrder> = game
            .state
            .processes
            .iter()
            .filter(|p| p.output == Output::Electricity || p.output == Output::Fuel)
            .map(|p| p.production_order(&agg_demand))
            .collect();
        let calorie_orders: Vec<ProductionOrder> = game
            .state
            .processes
            .iter()
        ...
        for order in energy_orders {
            energy_byproducts +=
                order.process.byproducts * order.amount / order.process.output_modifier;
        }
        ...
        let mut vals: Vec<String> = vec![
            ...
            (pop_demand.animal_calories + pop_demand.plant_calories)
                / game.state.world.population()
                / 365.,
            ...
            game.state.consumed_resources[Resource::Water] / consts::STARTING_RESOURCES.water
                * 100.,
        ...
        vals.extend(
            game.state
                ...
                .regions
                .iter()
                .map(|r| r.outlook.to_string()),
        );
        vals.extend(game.state.feedstocks.values().iter().map(|v| v.to_string()));
...
fn single_run(id: usize, rng: &mut SmallRng, scenarios: &Vec<Scenario>, dir: &str) {
    ...
    for i in 0..100 {
        ...
        for p_id in completed_projects {
            ...
            year_events.push((format!("🔶 Finished {}", project.name), None));
        }
        ...
        report.summary.push(Snapshot {
            ...
            land_use: game.state.consumed_resources[Resource::Land]
                / consts::STARTING_RESOURCES.land
                * 100.,
        });
    ...
    let event_names: Vec<&str> = content::events()
        .iter()

...
use crate::{HasId, Id, events::RegionFlag, flavor::RegionFlavor, kinds::*, outputs};
...
impl Region {
    pub fn develop(&mut self, speed: f32, stop: bool, degrow: bool) -> (usize, usize) {
        let start = self.income.level();
        if degrow && self.income == Income::High {
            self.develop_by(-1.);
        } else if !stop
            && self.income != Income::High
            && !(degrow && self.income == Income::UpperMiddle)
        {
    ...
    pub fn habitability(&self) -> f32 {
        ...
        (self.base_habitability - (f32::max(0., self.temp_hi - 35.).powf(2.) * 10.)).max(0.)
    }
    ...
    pub fn demand_level(&self, output: &Output, output_demand: &[OutputDemand; 4]) -> usize {
    ...
    pub fn demand_levels(&self, output_demand: &[OutputDemand; 4]) -> OutputMap {
    ...
    pub fn demand(&self, output_demand: &[OutputDemand; 4]) -> OutputMap {
    ...
    pub fn temp_range(&self) -> String {
        format!("{}-{}°C", self.temp_lo.round(), self.temp_hi.round())
    }

    pub fn precip_range(&self) -> String {
        format!("{}-{}cm/yr", self.precip_lo.round(), self.precip_hi.round())
    }

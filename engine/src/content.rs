// Do not edit this file. Use `editor/parse_content.py` to regenerate it.

use crate::effects::Effect;
use crate::projects::Project;
use crate::industries::Industry;
use crate::events::{Event, Choice};
use crate::regions::{Region, Income};
use crate::probability::{Probability, Likelihood};
use crate::condition::{Condition, Comparator, Flag};
use crate::production::{Process, ProcessFeature, Feedstock};
use crate::kinds::{Resource, Output, ByproductMap, ResourceMap};
use crate::variables::{WorldVariable, LocalVariable, PlayerVariable};
use crate::projects::Status as ProjectStatus;

const LAND_LIMIT: f32 = 104000000000.0;
const WATER_LIMIT: f32 = 45500000000000000.0;
const COAL_RESERVES: f32 = 1.074;
const URANIUM_RESERVES: f32 = 7988600.0;
const LITHIUM_RESERVES: f32 = 80000000.0;
const OIL_RESERVES: f32 = 275370680000000.0;
const WATER_BY_INCOME: [f32; 4] = [
    2040.4095,
    4552.624175,
    5839.79276,
    11648.18348
];
const ELECTRICITY_BY_INCOME: [f32; 4] = [
    12.647,
    80.881867,
    368.96432,
    750.5439
];
const FUEL_BY_INCOME: [f32; 4] = [
    84.823,
    478.748,
    1842.78168 ,
    4111.4561
];
const PLANT_CALORIES_BY_INCOME: [f32; 4] = [
    65101.22,
    69593.2533,
    73597.3736,
    75062.28
];
const ANIMAL_CALORIES_BY_INCOME: [f32; 4] = [
    5188.17,
    8453.8519,
    19610.3814,
    26695.9236
];
const MATERIALS_BY_INCOME: [f32; 4] = [
    2.253141687346895,
    4.3768,
    15.430,
    25.9541
];
const INCOME_POP_CHANGE_COEFS: [[f32; 4]; 4] = [
    [
        0.02510843736506849,
        -0.00027392308142168935,
        -1.2013335809592668e-06,
        1.6107860430271117e-08
    ],
    [
        0.015956334414138754,
        -0.00029336051833737166,
        7.154489844717687e-07,
        4.267315072469537e-09
    ],
    [
        0.007324313878541907,
        -0.00021522025711188524,
        2.7208734051441963e-09,
        9.029389058532674e-09
    ],
    [
        0.005554574166810743,
        -0.00032696397778050114,
        4.6328142827097576e-06,
        -2.1135532274204256e-08
    ]
];

const REGIONS: [Region; 20] = [
    Region {
        name: "Northern America",
        income: Income::High,
        health: 80.15,
        outlook: 100.0,
        population: 373318358.0,
        base_habitability: 100.0
    },
    Region {
        name: "Central America",
        income: Income::LowerMiddle,
        health: 63.45,
        outlook: 100.0,
        population: 183739076.0,
        base_habitability: 100.0
    },
    Region {
        name: "Southern America",
        income: Income::UpperMiddle,
        health: 64.8,
        outlook: 100.0,
        population: 437694436.0,
        base_habitability: 100.0
    },
    Region {
        name: "Caribbean",
        income: Income::High,
        health: 63.1,
        outlook: 100.0,
        population: 43977555.0,
        base_habitability: 100.0
    },
    Region {
        name: "Northern Africa",
        income: Income::LowerMiddle,
        health: 62.5,
        outlook: 100.0,
        population: 254985181.0,
        base_habitability: 100.0
    },
    Region {
        name: "Middle Africa",
        income: Income::LowerMiddle,
        health: 43.5,
        outlook: 100.0,
        population: 190463603.0,
        base_habitability: 100.0
    },
    Region {
        name: "Western Africa",
        income: Income::Low,
        health: 44.3,
        outlook: 100.0,
        population: 423239061.0,
        base_habitability: 100.0
    },
    Region {
        name: "Eastern Africa",
        income: Income::Low,
        health: 44.45,
        outlook: 100.0,
        population: 468849237.0,
        base_habitability: 100.0
    },
    Region {
        name: "Southern Africa",
        income: Income::UpperMiddle,
        health: 51.1,
        outlook: 100.0,
        population: 69191678.0,
        base_habitability: 100.0
    },
    Region {
        name: "Western Asia",
        income: Income::UpperMiddle,
        health: 76.35,
        outlook: 100.0,
        population: 288359967.0,
        base_habitability: 100.0
    },
    Region {
        name: "Central Asia",
        income: Income::LowerMiddle,
        health: 60.4,
        outlook: 100.0,
        population: 76474800.0,
        base_habitability: 100.0
    },
    Region {
        name: "Southern Asia",
        income: Income::LowerMiddle,
        health: 51.7,
        outlook: 100.0,
        population: 1983914039.0,
        base_habitability: 100.0
    },
    Region {
        name: "South-eastern Asia",
        income: Income::UpperMiddle,
        health: 52.0,
        outlook: 100.0,
        population: 681491532.0,
        base_habitability: 100.0
    },
    Region {
        name: "Eastern Asia",
        income: Income::UpperMiddle,
        health: 75.9,
        outlook: 100.0,
        population: 1686247807.0,
        base_habitability: 100.0
    },
    Region {
        name: "Oceania",
        income: Income::UpperMiddle,
        health: 76.14,
        outlook: 100.0,
        population: 12788202.0,
        base_habitability: 100.0
    },
    Region {
        name: "Western Europe",
        income: Income::High,
        health: 88.2,
        outlook: 100.0,
        population: 196908712.0,
        base_habitability: 100.0
    },
    Region {
        name: "Southern Europe",
        income: Income::High,
        health: 84.5,
        outlook: 100.0,
        population: 151644503.0,
        base_habitability: 100.0
    },
    Region {
        name: "Eastern Europe",
        income: Income::UpperMiddle,
        health: 74.4,
        outlook: 100.0,
        population: 291862973.0,
        base_habitability: 100.0
    },
    Region {
        name: "Northern Europe",
        income: Income::High,
        health: 87.1,
        outlook: 100.0,
        population: 106951178.0,
        base_habitability: 100.0
    },
    Region {
        name: "Australasia",
        income: Income::High,
        health: 89.22,
        outlook: 100.0,
        population: 30966994.0,
        base_habitability: 100.0
    }
];

const INDUSTRIES: [Industry; 8] = [
    Industry {
        name: "Iron and Steel",
        resources: resources!(
            electricity: 0.0,
            fuel: 22.19,
            land: 0.0,
            water: 0.0
        ),
        byproducts: byproducts!(
            ch4: 0.0,
            co2: 0.0,
            n2o: 0.0
        )
    },
    Industry {
        name: "Road Transport",
        resources: resources!(
            electricity: 0.0,
            fuel: 135.68,
            land: 0.0,
            water: 0.0
        ),
        byproducts: byproducts!(
            ch4: 0.0,
            co2: 0.0,
            n2o: 0.0
        )
    },
    Industry {
        name: "Aviation",
        resources: resources!(
            electricity: 0.0,
            fuel: 23.72,
            land: 0.0,
            water: 0.0
        ),
        byproducts: byproducts!(
            ch4: 0.0,
            co2: 0.0,
            n2o: 0.0
        )
    },
    Industry {
        name: "Shipping",
        resources: resources!(
            electricity: 0.0,
            fuel: 20.68,
            land: 0.0,
            water: 0.0
        ),
        byproducts: byproducts!(
            ch4: 0.0,
            co2: 0.0,
            n2o: 0.0
        )
    },
    Industry {
        name: "Chemical",
        resources: resources!(
            electricity: 0.0,
            fuel: 4.9,
            land: 0.0,
            water: 0.0
        ),
        byproducts: byproducts!(
            co2: 6.38,
            ch4: 0.0,
            n2o: 0.0
        )
    },
    Industry {
        name: "Concrete",
        resources: resources!(
            electricity: 0.0,
            fuel: 14.16,
            water: 0.0
        ),
        byproducts: byproducts!(
            ch4: 0.0,
            co2: 8.71,
            n2o: 0.0
        )
    },
    Industry {
        name: "Buildings",
        resources: resources!(
            electricity: 63.96,
            fuel: 65.95,
            land: 0.0,
            water: 425.0
        ),
        byproducts: byproducts!(
            co2: 0.0,
            ch4: 0.0,
            n2o: 0.0
        )
    },
    Industry {
        name: "Other Industry",
        resources: resources!(
            electricity: 58.47,
            fuel: 146.09,
            land: 0.0,
            water: 915.0
        ),
        byproducts: byproducts!(
            ch4: 0.0,
            co2: 0.0,
            n2o: 0.0
        )
    }
];

const PROCESSES: [Process; 29] = [
    Process {
        id: 0,
        name: "BECCS",
        output: Output::Electricity,
        mix_share: 2.5,
        feedstock: (Feedstock::Soil, 1.0),
        resources: resources!(
            electricity: 0.0925,
            fuel: 0.0926,
            land: 0.1268,
            water: 986.4
        ),
        byproducts: byproducts!(
            ch4: 0.0,
            co2: -1166.67,
            n2o: 0.06337
        ),
        locked: false,
        banned: false
    },
    Process {
        id: 1,
        name: "Nuclear Fusion",
        output: Output::Electricity,
        mix_share: 0.0,
        feedstock: (Feedstock::Lithium, 0.00012),
        resources: resources!(
            electricity: 4.6e-07,
            fuel: 0.0,
            land: 0.000119,
            water: 9.28e-05
        ),
        byproducts: byproducts!(
            ch4: 0.0,
            co2: 3.95e-05,
            n2o: 0.0
        ),
        locked: true,
        banned: false
    },
    Process {
        id: 2,
        name: "Cellular Meat",
        output: Output::AnimalCalories,
        mix_share: 0.0,
        feedstock: (Feedstock::Other, 1.0),
        resources: resources!(
            electricity: 0.2,
            fuel: 0.0,
            land: 0.00067,
            water: 0.018
        ),
        byproducts: byproducts!(
            ch4: 0.0,
            co2: 0.463,
            n2o: 0.0
        ),
        locked: true,
        banned: false
    },
    Process {
        id: 3,
        name: "Floating Wind Turbines",
        output: Output::Electricity,
        mix_share: 0.0,
        feedstock: (Feedstock::Other, 1.0),
        resources: resources!(
            electricity: 0.0556,
            fuel: 0.0,
            land: 0.0,
            water: 0.022
        ),
        byproducts: byproducts!(
            ch4: 0.0,
            co2: 10.0,
            n2o: 0.0
        ),
        locked: true,
        banned: false
    },
    Process {
        id: 4,
        name: "Regenerative Agriculture (Crops)",
        output: Output::PlantCalories,
        mix_share: 0.0,
        feedstock: (Feedstock::Soil, 1.0),
        resources: resources!(
            electricity: 0.0636,
            fuel: 0.0986,
            land: 0.001,
            water: 2.03
        ),
        byproducts: byproducts!(
            ch4: 0.0,
            co2: -43.347,
            n2o: 0.0
        ),
        locked: false,
        banned: false
    },
    Process {
        id: 5,
        name: "Regenerative Agriculture (Livestock)",
        output: Output::AnimalCalories,
        mix_share: 0.0,
        feedstock: (Feedstock::Soil, 1.0),
        resources: resources!(
            electricity: 0.52983,
            fuel: 0.82143,
            land: 0.0171,
            water: 5.29
        ),
        byproducts: byproducts!(
            ch4: 0.01456,
            co2: 0.52983,
            n2o: 0.0
        ),
        locked: false,
        banned: false
    },
    Process {
        id: 6,
        name: "Thorium Nuclear Power",
        output: Output::Electricity,
        mix_share: 0.0,
        feedstock: (Feedstock::Other, 0.0),
        resources: resources!(

        ),
        byproducts: byproducts!(

        ),
        locked: true,
        banned: false
    },
    Process {
        id: 7,
        name: "Breeder Nuclear Power",
        output: Output::Electricity,
        mix_share: 0.0,
        feedstock: (Feedstock::Uranium, 0.0),
        resources: resources!(

        ),
        byproducts: byproducts!(

        ),
        locked: true,
        banned: false
    },
    Process {
        id: 8,
        name: "Industrial Agriculture (Crops)",
        output: Output::PlantCalories,
        mix_share: 67.75,
        feedstock: (Feedstock::Soil, 1.0),
        resources: resources!(
            electricity: 0.042,
            fuel: 0.0651,
            land: 0.00066417,
            water: 1.34
        ),
        byproducts: byproducts!(
            ch4: 0.0,
            co2: 0.0771,
            n2o: 0.0001473
        ),
        locked: false,
        banned: false
    },
    Process {
        id: 9,
        name: "Organic Agriculture (Crops)",
        output: Output::PlantCalories,
        mix_share: 0.25,
        feedstock: (Feedstock::Soil, 1.0),
        resources: resources!(
            electricity: 0.0636,
            fuel: 0.0986,
            land: 0.001,
            water: 2.03
        ),
        byproducts: byproducts!(
            ch4: 0.0,
            co2: 0.1168,
            n2o: 0.0
        ),
        locked: false,
        banned: false
    },
    Process {
        id: 10,
        name: "Industrial Agriculture (Livestock)",
        output: Output::AnimalCalories,
        mix_share: 99.75,
        feedstock: (Feedstock::Soil, 1.0),
        resources: resources!(
            electricity: 0.34969,
            fuel: 0.54214,
            land: 0.01123,
            water: 3.489
        ),
        byproducts: byproducts!(
            ch4: 0.0096,
            co2: 0.6424,
            n2o: 0.001228
        ),
        locked: false,
        banned: false
    },
    Process {
        id: 11,
        name: "Organic Agriculture (Livestock)",
        output: Output::AnimalCalories,
        mix_share: 0.25,
        feedstock: (Feedstock::Soil, 1.0),
        resources: resources!(
            electricity: 0.52983,
            fuel: 0.82143,
            land: 0.0171,
            water: 5.29
        ),
        byproducts: byproducts!(
            ch4: 0.01456,
            co2: 0.97334,
            n2o: 0.0
        ),
        locked: false,
        banned: false
    },
    Process {
        id: 12,
        name: "Oil Power Generation",
        output: Output::Electricity,
        mix_share: 3.1,
        feedstock: (Feedstock::Oil, 0.3636872),
        resources: resources!(
            electricity: 0.1666667,
            fuel: 0.1666667,
            land: 0.00017562346,
            water: 3.68
        ),
        byproducts: byproducts!(
            co2: 1005.0,
            n2o: 0.0,
            ch4: 0.6191
        ),
        locked: false,
        banned: false
    },
    Process {
        id: 13,
        name: "Coal Power Generation",
        output: Output::Electricity,
        mix_share: 36.7,
        feedstock: (Feedstock::Coal, 512.559),
        resources: resources!(
            electricity: 0.00887,
            fuel: 0.00887,
            land: 0.000114,
            water: 0.00392
        ),
        byproducts: byproducts!(
            ch4: 0.57829,
            co2: 1000.0,
            n2o: 0.0
        ),
        locked: false,
        banned: false
    },
    Process {
        id: 14,
        name: "Natural Gas Power Generation",
        output: Output::Electricity,
        mix_share: 23.5,
        feedstock: (Feedstock::NaturalGas, 210.3942),
        resources: resources!(
            electricity: 0.01325,
            fuel: 0.01325,
            land: 2.54e-05,
            water: 2.3688
        ),
        byproducts: byproducts!(
            ch4: 2.7,
            co2: 410.9589,
            n2o: 0.0
        ),
        locked: false,
        banned: false
    },
    Process {
        id: 15,
        name: "Nuclear Power",
        output: Output::Electricity,
        mix_share: 10.4,
        feedstock: (Feedstock::Uranium, 0.0009),
        resources: resources!(
            electricity: 0.1,
            fuel: 0.1,
            land: 0.000119,
            water: 2.13666
        ),
        byproducts: byproducts!(
            ch4: 0.0,
            co2: 47.0,
            n2o: 0.0
        ),
        locked: false,
        banned: false
    },
    Process {
        id: 16,
        name: "Hydropower",
        output: Output::Electricity,
        mix_share: 15.8,
        feedstock: (Feedstock::Other, 1.0),
        resources: resources!(
            electricity: 0.01,
            fuel: 0.0,
            land: 0.0065,
            water: 1530.5
        ),
        byproducts: byproducts!(
            ch4: 0.0,
            co2: 9.0,
            n2o: 0.0
        ),
        locked: false,
        banned: false
    },
    Process {
        id: 17,
        name: "Terrestrial Wind Power",
        output: Output::Electricity,
        mix_share: 5.3,
        feedstock: (Feedstock::Other, 1.0),
        resources: resources!(
            electricity: 0.0556,
            fuel: 0.0,
            land: 0.00228,
            water: 0.022
        ),
        byproducts: byproducts!(
            ch4: 0.0,
            co2: 10.0,
            n2o: 0.0
        ),
        locked: false,
        banned: false
    },
    Process {
        id: 18,
        name: "Solar PV",
        output: Output::Electricity,
        mix_share: 2.65,
        feedstock: (Feedstock::Other, 1.0),
        resources: resources!(
            electricity: 0.083,
            fuel: 0.0,
            land: 0.0114,
            water: 0.557
        ),
        byproducts: byproducts!(
            ch4: 0.0,
            co2: 40.0,
            n2o: 0.0
        ),
        locked: false,
        banned: false
    },
    Process {
        id: 19,
        name: "Oil",
        output: Output::Fuel,
        mix_share: 52.67,
        feedstock: (Feedstock::Oil, 0.3636872),
        resources: resources!(
            land: 0.00017562346,
            water: 3.68,
            fuel: 0.1666667,
            electricity: 0.1666667
        ),
        byproducts: byproducts!(
            co2: 1005.0,
            ch4: 0.6191,
            n2o: 0.0
        ),
        locked: false,
        banned: false
    },
    Process {
        id: 20,
        name: "Natural Gas",
        output: Output::Fuel,
        mix_share: 21.38,
        feedstock: (Feedstock::NaturalGas, 210.3942),
        resources: resources!(
            land: 2.54e-05,
            water: 2.3688,
            fuel: 0.01325,
            electricity: 0.01325
        ),
        byproducts: byproducts!(
            co2: 410.9589,
            ch4: 2.7,
            n2o: 0.0
        ),
        locked: false,
        banned: false
    },
    Process {
        id: 21,
        name: "Biofuels",
        output: Output::Fuel,
        mix_share: 13.56,
        feedstock: (Feedstock::Soil, 1.0),
        resources: resources!(
            land: 0.1268,
            water: 986.4,
            fuel: 0.0926,
            electricity: 0.0926
        ),
        byproducts: byproducts!(
            n2o: 0.06337,
            ch4: 0.0,
            co2: 699.488
        ),
        locked: false,
        banned: false
    },
    Process {
        id: 22,
        name: "Coal",
        output: Output::Fuel,
        mix_share: 12.39,
        feedstock: (Feedstock::Coal, 512.559),
        resources: resources!(
            land: 0.000114,
            water: 0.00392,
            fuel: 0.00887,
            electricity: 0.00887
        ),
        byproducts: byproducts!(
            co2: 1000.0,
            n2o: 0.0,
            ch4: 0.57829
        ),
        locked: false,
        banned: false
    },
    Process {
        id: 23,
        name: "Vertical Farming",
        output: Output::PlantCalories,
        mix_share: 0.0,
        feedstock: (Feedstock::Other, 1.0),
        resources: resources!(
            electricity: 16.785,
            fuel: 0.0,
            land: 6.3178e-05,
            water: 0.1073
        ),
        byproducts: byproducts!(
            ch4: 0.0,
            co2: 0.0502,
            n2o: 0.0
        ),
        locked: false,
        banned: false
    },
    Process {
        id: 24,
        name: "Smallholder Farms",
        output: Output::PlantCalories,
        mix_share: 32.0,
        feedstock: (Feedstock::Soil, 1.0),
        resources: resources!(
            electricity: 0.03468,
            fuel: 0.053766,
            land: 0.0005489,
            water: 1.1085
        ),
        byproducts: byproducts!(
            ch4: 0.0,
            co2: 0.06371,
            n2o: 0.0
        ),
        locked: false,
        banned: false
    },
    Process {
        id: 25,
        name: "Grey Hydrogen",
        output: Output::Fuel,
        mix_share: 0.0,
        feedstock: (Feedstock::NaturalGas, 0.0),
        resources: resources!(

        ),
        byproducts: byproducts!(

        ),
        locked: false,
        banned: false
    },
    Process {
        id: 26,
        name: "Blue Hydrogen",
        output: Output::Fuel,
        mix_share: 0.0,
        feedstock: (Feedstock::NaturalGas, 0.0),
        resources: resources!(

        ),
        byproducts: byproducts!(

        ),
        locked: false,
        banned: false
    },
    Process {
        id: 27,
        name: "Green Hydrogen",
        output: Output::Fuel,
        mix_share: 0.0,
        feedstock: (Feedstock::Other, 0.0),
        resources: resources!(

        ),
        byproducts: byproducts!(

        ),
        locked: false,
        banned: false
    },
    Process {
        id: 28,
        name: "Concentrated Solar Power",
        output: Output::Electricity,
        mix_share: 0.05,
        feedstock: (Feedstock::Other, 1.0),
        resources: resources!(
            electricity: 0.625,
            fuel: 0.0,
            land: 0.0057,
            water: 0.00414
        ),
        byproducts: byproducts!(
            ch4: 0.0,
            co2: 31.2,
            n2o: 0.0
        ),
        locked: false,
        banned: false
    }
];

const PROJECTS: [Project; 54] = [
    Project {
        id: 0,
        name: "Cloud Brightening",
        years: 1,
        effects: vec![

        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: true,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 1,
        name: "Veganism Mandate",
        years: 1,
        effects: vec![

        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 2,
        name: "Remediation and Restoration",
        years: 1,
        effects: vec![
            Effect::UnlocksProject(38)
        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: true,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 3,
        name: "Nuclear Fusion",
        years: 20,
        effects: vec![

        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 4,
        name: "Perennial Cereals",
        years: 5,
        effects: vec![
            Effect::Output(Output::PlantCalories, 0.)
        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 5,
        name: "Universal Abortions & Contraceptives",
        years: 1,
        effects: vec![

        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 6,
        name: "One-Child Policy",
        years: 1,
        effects: vec![

        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 7,
        name: "Indigenous Land Management",
        years: 1,
        effects: vec![

        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 8,
        name: "Battery Storage Network",
        years: 1,
        effects: vec![

        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: true,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 9,
        name: "Solar Radiation Management",
        years: 1,
        effects: vec![
            Effect::OutputForFeature(ProcessFeature::IsSolar, -10.0)
        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: true,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 10,
        name: "Expand & Upgrade Public Transit",
        years: 5,
        effects: vec![

        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 11,
        name: "Long-Range Electric Aviation",
        years: 10,
        effects: vec![

        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 12,
        name: "Closed Borders",
        years: 1,
        effects: vec![

        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: true,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 13,
        name: "Iron Fillings",
        years: 10,
        effects: vec![

        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 14,
        name: "Green Hydrogen",
        years: 10,
        effects: vec![
            Effect::UnlocksProject(15)
        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 15,
        name: "Hydrogen Transport Network",
        years: 10,
        effects: vec![

        ],
        locked: true,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 16,
        name: "III-V Semiconductors solar panels and perovskite-based panels",
        years: 10,
        effects: vec![

        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 17,
        name: "Floating Wind Turbines",
        years: 10,
        effects: vec![
            Effect::UnlocksProcess(3)
        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 18,
        name: "Battery Technology?",
        years: 10,
        effects: vec![

        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 19,
        name: "De-extinction",
        years: 10,
        effects: vec![
            Effect::UnlocksProject(20)
        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 20,
        name: "De-extinction Initiative",
        years: 10,
        effects: vec![

        ],
        locked: true,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 21,
        name: "Global Passive House Mandate",
        years: 10,
        effects: vec![

        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 22,
        name: "Car Ban",
        years: 10,
        effects: vec![

        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 23,
        name: "Direct Air Capture",
        years: 20,
        effects: vec![

        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: true,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 24,
        name: "Co-Generation",
        years: 10,
        effects: vec![

        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 25,
        name: "Restrictions on Air Travel",
        years: 2,
        effects: vec![

        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 26,
        name: "Wood Skyscrapers",
        years: 5,
        effects: vec![

        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 27,
        name: "Vegetarian Mandate",
        years: 10,
        effects: vec![

        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 28,
        name: "Back to the Land",
        years: 10,
        effects: vec![

        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 29,
        name: "3rd Generation Biofuels",
        years: 10,
        effects: vec![

        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 30,
        name: "Thorium Reactor",
        years: 20,
        effects: vec![
            Effect::UnlocksProcess(6)
        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 31,
        name: "Breeder Reactor",
        years: 10,
        effects: vec![
            Effect::UnlocksProcess(7)
        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 32,
        name: "Mini Nuclear Reactor",
        years: 10,
        effects: vec![

        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 33,
        name: "Asteroid Mining",
        years: 15,
        effects: vec![

        ],
        locked: true,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 34,
        name: "Closed-System Ecologies (Biosphere III)",
        years: 10,
        effects: vec![

        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 35,
        name: "Space Cans",
        years: 10,
        effects: vec![

        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 36,
        name: "Traditional animal husbandry mandate",
        years: 5,
        effects: vec![

        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 37,
        name: "Land Expansion",
        years: 10,
        effects: vec![

        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 38,
        name: "Phytomining",
        years: 10,
        effects: vec![

        ],
        locked: true,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 39,
        name: "Cloud Seeding",
        years: 10,
        effects: vec![

        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 40,
        name: "Ban on deep sea mining",
        years: 10,
        effects: vec![

        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 41,
        name: "Agricultural robotics",
        years: 10,
        effects: vec![

        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 42,
        name: "The Ark",
        years: 10,
        effects: vec![

        ],
        locked: true,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 43,
        name: "Expand recycling infrastructure",
        years: 10,
        effects: vec![

        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 44,
        name: "Alternative Refrigerants",
        years: 10,
        effects: vec![

        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 45,
        name: "Drought-Resistant Crop Varieties",
        years: 10,
        effects: vec![

        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 46,
        name: "Electrification",
        years: 10,
        effects: vec![

        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 47,
        name: "Hempcrete",
        years: 10,
        effects: vec![

        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 48,
        name: "Orbital Mirror Array",
        years: 20,
        effects: vec![

        ],
        locked: true,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 49,
        name: "Carbon-Negative Concrete",
        years: 10,
        effects: vec![

        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 50,
        name: "Hydrogen Steel",
        years: 10,
        effects: vec![

        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 51,
        name: "Electric-Arc Furnaces",
        years: 10,
        effects: vec![

        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 52,
        name: "Cellular Meat",
        years: 20,
        effects: vec![
            Effect::UnlocksProcess(2)
        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    },
    Project {
        id: 53,
        name: "Mass CCS",
        years: 10,
        effects: vec![

        ],
        locked: false,
        status: ProjectStatus::Inactive,
        ongoing: false,
        resources: resources!(

        ),
        byproducts: byproducts!(

        )
    }
];

const EVENTS: [Event; 81] = [
    Event {
        id: 0,
        name: "Heatwaves",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::LocalVariable(LocalVariable::Health, -10.0),
            Effect::LocalVariable(LocalVariable::Population, -1.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Likely,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Temperature, Comparator::GreaterEqual, 2.5)
                ]
            },
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Temperature, Comparator::GreaterEqual, 2.0)
                ]
            },
            Probability {
                likelihood: Likelihood::Unlikely,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Temperature, Comparator::GreaterEqual, 1.5)
                ]
            },
            Probability {
                likelihood: Likelihood::Rare,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Temperature, Comparator::GreaterEqual, 1.0)
                ]
            }
        ],
        choices: vec![
            Choice {
                effects: vec![
                    Effect::Resource(Resource::Labor, -20.0),
                    Effect::LocalVariable(LocalVariable::Population, -1.0)
                ],
                conditions: vec![

                ]
            },
            Choice {
                effects: vec![
                    Effect::LocalVariable(LocalVariable::Population, -2.0)
                ],
                conditions: vec![

                ]
            }
        ]
    },
    Event {
        id: 1,
        name: "Crop Failures",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![

        ],
        probabilities: vec![

        ],
        choices: vec![

        ]
    },
    Event {
        id: 2,
        name: "Great Barrier Reef declared dead",
        locked: false,
        local: true,
        repeats: false,
        effects: vec![
            Effect::WorldVariable(WorldVariable::Biodiversity, -5.0),
            Effect::WorldVariable(WorldVariable::Outlook, -5.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Guaranteed,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Temperature, Comparator::GreaterEqual, 2.5)
                ]
            },
            Probability {
                likelihood: Likelihood::Likely,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Temperature, Comparator::GreaterEqual, 2.0)
                ]
            },
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Temperature, Comparator::GreaterEqual, 1.5)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 3,
        name: "Novel Disease",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::LocalVariable(LocalVariable::Population, -1.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Rare,
                conditions: vec![
                    Condition::ProcessMixShareFeature(ProcessFeature::UsesLivestock, Comparator::Greater, 0.2)
                ]
            },
            Probability {
                likelihood: Likelihood::Unlikely,
                conditions: vec![
                    Condition::ProcessMixShareFeature(ProcessFeature::UsesLivestock, Comparator::Greater, 0.4)
                ]
            },
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::Resource(Resource::Land, Comparator::GreaterEqual, 0.65)
                ]
            },
            Probability {
                likelihood: Likelihood::Unlikely,
                conditions: vec![
                    Condition::Resource(Resource::Land, Comparator::GreaterEqual, 0.5)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 4,
        name: "Species Flourishing",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::WorldVariable(WorldVariable::Outlook, 5.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Biodiversity, Comparator::GreaterEqual, 0.9)
                ]
            },
            Probability {
                likelihood: Likelihood::Unlikely,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Biodiversity, Comparator::Greater, 0.8)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 5,
        name: "Species Collapse",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::WorldVariable(WorldVariable::Outlook, -5.0),
            Effect::WorldVariable(WorldVariable::Biodiversity, -1.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Likely,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Biodiversity, Comparator::LessEqual, 0.4)
                ]
            },
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Biodiversity, Comparator::LessEqual, 0.6)
                ]
            },
            Probability {
                likelihood: Likelihood::Unlikely,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Biodiversity, Comparator::LessEqual, 0.8)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 6,
        name: "Land restoration programs backtrack",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::WorldVariable(WorldVariable::Emissions, 1.0),
            Effect::WorldVariable(WorldVariable::Biodiversity, -1.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Likely,
                conditions: vec![
                    Condition::ProjectStatus(2, ProjectStatus::Stalled)
                ]
            },
            Probability {
                likelihood: Likelihood::Likely,
                conditions: vec![
                    Condition::ProjectStatus(2, ProjectStatus::Halted)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 7,
        name: "Unrest - Protest",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::AddEvent(8)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Guaranteed,
                conditions: vec![
                    Condition::LocalVariable(LocalVariable::Contentedness, Comparator::LessEqual, 0.25)
                ]
            },
            Probability {
                likelihood: Likelihood::Likely,
                conditions: vec![
                    Condition::LocalVariable(LocalVariable::Contentedness, Comparator::LessEqual, 0.4)
                ]
            },
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::LocalVariable(LocalVariable::Contentedness, Comparator::LessEqual, 0.6)
                ]
            },
            Probability {
                likelihood: Likelihood::Unlikely,
                conditions: vec![
                    Condition::LocalVariable(LocalVariable::Contentedness, Comparator::LessEqual, 0.8)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 8,
        name: "Unrest - Riots",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::AddEvent(10)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Guaranteed,
                conditions: vec![
                    Condition::LocalVariable(LocalVariable::Contentedness, Comparator::LessEqual, 0.1)
                ]
            },
            Probability {
                likelihood: Likelihood::Likely,
                conditions: vec![
                    Condition::LocalVariable(LocalVariable::Contentedness, Comparator::LessEqual, 0.25)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 9,
        name: "Wildfires - Severe",
        locked: false,
        local: true,
        repeats: true,
        effects: vec![
            Effect::AddEvent(11),
            Effect::AddEvent(13),
            Effect::WorldVariable(WorldVariable::Emissions, 1.0),
            Effect::LocalVariable(LocalVariable::Outlook, -1.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Impossible,
                conditions: vec![
                    Condition::ProjectStatus(7, ProjectStatus::Active)
                ]
            },
            Probability {
                likelihood: Likelihood::Likely,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Temperature, Comparator::Greater, 1.5)
                ]
            },
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Temperature, Comparator::Greater, 1.0)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 10,
        name: "Unrest - Revolts",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::TriggerEvent(12, 4)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Guaranteed,
                conditions: vec![
                    Condition::LocalVariable(LocalVariable::Contentedness, Comparator::LessEqual, 0.1)
                ]
            },
            Probability {
                likelihood: Likelihood::Likely,
                conditions: vec![
                    Condition::LocalVariable(LocalVariable::Contentedness, Comparator::LessEqual, 0.15)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 11,
        name: "Wildfires - Smoke",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::OutputForFeature(ProcessFeature::IsSolar, -5.0),
            Effect::Output(Output::PlantCalories, -2.0),
            Effect::LocalVariable(LocalVariable::Health, -1.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Likely,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Temperature, Comparator::Greater, 1.0)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 12,
        name: "Unrest - Secession",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::PlayerVariable(PlayerVariable::PoliticalCapital, -100.0),
            Effect::RegionLeave
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Guaranteed,
                conditions: vec![
                    Condition::LocalVariable(LocalVariable::Contentedness, Comparator::LessEqual, 0.2)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 13,
        name: "Wildfires - Ancient Loss",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::WorldVariable(WorldVariable::Outlook, -5.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Likely,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Temperature, Comparator::GreaterEqual, 1.5)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 14,
        name: "Wet-Bulb Event",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::WorldVariable(WorldVariable::Population, -5.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Temperature, Comparator::GreaterEqual, 2.0)
                ]
            },
            Probability {
                likelihood: Likelihood::Rare,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Temperature, Comparator::GreaterEqual, 1.5)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 15,
        name: "Permafrost Melting",
        locked: false,
        local: true,
        repeats: true,
        effects: vec![
            Effect::WorldVariable(WorldVariable::Emissions, 10.0),
            Effect::AddEvent(16)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Unlikely,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Temperature, Comparator::GreaterEqual, 2.0)
                ]
            },
            Probability {
                likelihood: Likelihood::Likely,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Temperature, Comparator::GreaterEqual, 3.0)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 16,
        name: "Permafrost Contagion",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::WorldVariable(WorldVariable::Outlook, -2.0),
            Effect::WorldVariable(WorldVariable::Population, -0.5)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Improbable,
                conditions: vec![

                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 17,
        name: "Fish Depletion",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::Demand(Output::PlantCalories, 10.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Temperature, Comparator::GreaterEqual, 1.5)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 18,
        name: "Massive Thunderstorms",
        locked: false,
        local: true,
        repeats: true,
        effects: vec![
            Effect::Output(Output::Electricity, -5.0),
            Effect::LocalVariable(LocalVariable::Health, -1.0),
            Effect::LocalVariable(LocalVariable::Outlook, -1.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Temperature, Comparator::GreaterEqual, 1.5)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 19,
        name: "Blue Ocean Event",
        locked: false,
        local: true,
        repeats: true,
        effects: vec![
            Effect::WorldVariable(WorldVariable::SeaLevelRise, 0.),
            Effect::WorldVariable(WorldVariable::Outlook, 0.),
            Effect::Feedstock(Feedstock::Oil, 10.0)
        ],
        probabilities: vec![

        ],
        choices: vec![

        ]
    },
    Event {
        id: 20,
        name: "Mosquito Disease",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::LocalVariable(LocalVariable::Health, -1.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Biodiversity, Comparator::LessEqual, 0.5)
                ]
            },
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Temperature, Comparator::GreaterEqual, 1.5)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 21,
        name: "Brownouts",
        locked: false,
        local: true,
        repeats: true,
        effects: vec![
            Effect::LocalVariable(LocalVariable::Health, -1.0),
            Effect::LocalVariable(LocalVariable::Outlook, -1.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Likely,
                conditions: vec![
                    Condition::ProcessMixShareFeature(ProcessFeature::IsIntermittent, Comparator::GreaterEqual, 0.6),
                    Condition::ProjectStatus(8, ProjectStatus::Inactive)
                ]
            },
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::ProcessMixShareFeature(ProcessFeature::IsIntermittent, Comparator::GreaterEqual, 0.4),
                    Condition::ProjectStatus(8, ProjectStatus::Inactive)
                ]
            },
            Probability {
                likelihood: Likelihood::Unlikely,
                conditions: vec![
                    Condition::ProcessMixShareFeature(ProcessFeature::IsIntermittent, Comparator::GreaterEqual, 0.25),
                    Condition::ProjectStatus(8, ProjectStatus::Inactive)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 22,
        name: "Nuclear Meltdown",
        locked: false,
        local: true,
        repeats: true,
        effects: vec![
            Effect::LocalVariable(LocalVariable::Health, -3.0),
            Effect::LocalVariable(LocalVariable::Outlook, -3.0),
            Effect::OutputForFeature(ProcessFeature::IsNuclear, -5.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::ProcessMixShareFeature(ProcessFeature::IsNuclear, Comparator::GreaterEqual, 0.5)
                ]
            },
            Probability {
                likelihood: Likelihood::Unlikely,
                conditions: vec![
                    Condition::ProcessMixShareFeature(ProcessFeature::IsNuclear, Comparator::GreaterEqual, 0.2)
                ]
            },
            Probability {
                likelihood: Likelihood::Rare,
                conditions: vec![
                    Condition::ProcessMixShareFeature(ProcessFeature::IsNuclear, Comparator::Greater, 0.)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 23,
        name: "Nuclear Waste",
        locked: false,
        local: true,
        repeats: true,
        effects: vec![
            Effect::LocalVariable(LocalVariable::Health, -1.0),
            Effect::LocalVariable(LocalVariable::Outlook, -1.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::ProcessMixShareFeature(ProcessFeature::IsNuclear, Comparator::GreaterEqual, 0.5)
                ]
            },
            Probability {
                likelihood: Likelihood::Unlikely,
                conditions: vec![
                    Condition::ProcessMixShareFeature(ProcessFeature::IsNuclear, Comparator::GreaterEqual, 0.2)
                ]
            },
            Probability {
                likelihood: Likelihood::Rare,
                conditions: vec![
                    Condition::ProcessMixShareFeature(ProcessFeature::IsNuclear, Comparator::Greater, 0.)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 24,
        name: "Species Death",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::WorldVariable(WorldVariable::Outlook, -1.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Biodiversity, Comparator::LessEqual, 0.5)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 25,
        name: "Flooding",
        locked: false,
        local: true,
        repeats: true,
        effects: vec![
            Effect::LocalVariable(LocalVariable::Health, -1.0),
            Effect::LocalVariable(LocalVariable::Outlook, -1.0),
            Effect::TriggerEvent(61, 1)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Temperature, Comparator::GreaterEqual, 2.0)
                ]
            },
            Probability {
                likelihood: Likelihood::Unlikely,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Temperature, Comparator::GreaterEqual, 1.5)
                ]
            },
            Probability {
                likelihood: Likelihood::Rare,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Temperature, Comparator::GreaterEqual, 1.0)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 26,
        name: "Ozone Depletion",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::TriggerEvent(39, 12),
            Effect::TriggerEvent(38, 18)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Unlikely,
                conditions: vec![
                    Condition::ProjectStatus(9, ProjectStatus::Active)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 27,
        name: "Migrations - No Borders",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::Migration
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Guaranteed,
                conditions: vec![
                    Condition::LocalVariable(LocalVariable::Habitability, Comparator::LessEqual, 0.1),
                    Condition::ProjectStatus(12, ProjectStatus::Inactive)
                ]
            },
            Probability {
                likelihood: Likelihood::Likely,
                conditions: vec![
                    Condition::LocalVariable(LocalVariable::Habitability, Comparator::LessEqual, 0.5),
                    Condition::ProjectStatus(12, ProjectStatus::Inactive)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 28,
        name: "Island Abandoned",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::WorldVariable(WorldVariable::Outlook, -2.0),
            Effect::Migration
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Guaranteed,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::SeaLevelRise, Comparator::GreaterEqual, 5.0)
                ]
            },
            Probability {
                likelihood: Likelihood::Likely,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::SeaLevelRise, Comparator::GreaterEqual, 4.0)
                ]
            },
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::SeaLevelRise, Comparator::GreaterEqual, 3.0)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 29,
        name: "SRM - SAD",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::WorldVariable(WorldVariable::Outlook, -2.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::ProjectStatus(9, ProjectStatus::Active)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 30,
        name: "Urban Gardening",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::Demand(Output::PlantCalories, -10.0),
            Effect::AddEvent(31)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![

                ]
            },
            Probability {
                likelihood: Likelihood::Likely,
                conditions: vec![
                    Condition::OutputDemandGap(Output::PlantCalories, Comparator::GreaterEqual, 0.25)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 31,
        name: "Urban Farming",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::Demand(Output::PlantCalories, -10.0),
            Effect::WorldVariable(WorldVariable::Outlook, 1.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Likely,
                conditions: vec![
                    Condition::ProjectStatus(10, ProjectStatus::Finished),
                    Condition::OutputDemandGap(Output::PlantCalories, Comparator::GreaterEqual, 0.1)
                ]
            },
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::ProjectStatus(22, ProjectStatus::Finished)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 32,
        name: "SRM - Crop Yields",
        locked: false,
        local: true,
        repeats: true,
        effects: vec![
            Effect::Output(Output::PlantCalories, -5.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::ProjectStatus(9, ProjectStatus::Active)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 33,
        name: "UFO Visit",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::TriggerEvent(34, 6),
            Effect::TriggerEvent(35, 6)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Improbable,
                conditions: vec![
                    Condition::RunsPlayed(Comparator::GreaterEqual, 1)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 34,
        name: "UFO Gift",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::WorldVariable(WorldVariable::Emissions, -50.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Guaranteed,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Biodiversity, Comparator::GreaterEqual, 0.7)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 35,
        name: "UFO War",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::WorldVariable(WorldVariable::Population, -50.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Guaranteed,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Biodiversity, Comparator::Less, 0.7)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 36,
        name: "New Cuisine",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::WorldVariable(WorldVariable::Outlook, 1.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Temperature, Comparator::GreaterEqual, 1.5)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 37,
        name: "Meat Militia",
        locked: false,
        local: true,
        repeats: true,
        effects: vec![
            Effect::WorldVariable(WorldVariable::Outlook, -1.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Unlikely,
                conditions: vec![
                    Condition::ProjectStatus(1, ProjectStatus::Active)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 38,
        name: "Plant UV Exposure",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::Output(Output::PlantCalories, -5.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::ProjectStatus(9, ProjectStatus::Active)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 39,
        name: "Ozone Health",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::WorldVariable(WorldVariable::Health, -3.0),
            Effect::AddEvent(73)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::ProjectStatus(9, ProjectStatus::Active)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 40,
        name: "Migrations - Borders",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::LocalVariable(LocalVariable::Population, -1.0),
            Effect::LocalVariable(LocalVariable::Outlook, -1.0),
            Effect::TriggerEvent(41, 6)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Guaranteed,
                conditions: vec![
                    Condition::LocalVariable(LocalVariable::Habitability, Comparator::LessEqual, 0.1),
                    Condition::ProjectStatus(12, ProjectStatus::Active)
                ]
            },
            Probability {
                likelihood: Likelihood::Likely,
                conditions: vec![
                    Condition::LocalVariable(LocalVariable::Habitability, Comparator::LessEqual, 0.5),
                    Condition::ProjectStatus(12, ProjectStatus::Active)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 41,
        name: "Migrations - Border Attack",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::LocalVariable(LocalVariable::Population, -2.0),
            Effect::LocalVariable(LocalVariable::Outlook, -2.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Guaranteed,
                conditions: vec![
                    Condition::LocalVariable(LocalVariable::Habitability, Comparator::LessEqual, 0.1),
                    Condition::ProjectStatus(12, ProjectStatus::Active)
                ]
            },
            Probability {
                likelihood: Likelihood::Likely,
                conditions: vec![
                    Condition::LocalVariable(LocalVariable::Habitability, Comparator::LessEqual, 0.3),
                    Condition::ProjectStatus(12, ProjectStatus::Active)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 42,
        name: "Doom Cult - Formation",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::TriggerEvent(43, 6)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Outlook, Comparator::LessEqual, 0.25)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 43,
        name: "Doom Cult - Spreading",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::WorldVariable(WorldVariable::Outlook, -0.5),
            Effect::TriggerEvent(44, 6)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Outlook, Comparator::LessEqual, 0.25)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 44,
        name: "Doom Cult - Acceleration",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::WorldVariable(WorldVariable::Outlook, -0.5),
            Effect::TriggerEvent(45, 6)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Outlook, Comparator::LessEqual, 0.1)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 45,
        name: "Doom Cult - Attacks",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::WorldVariable(WorldVariable::Outlook, -2.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Outlook, Comparator::LessEqual, 0.1)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 46,
        name: "Rivals - Formation",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![

        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Impossible,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Emissions, Comparator::Greater, 0.)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 47,
        name: "Rerualization",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![

        ],
        probabilities: vec![

        ],
        choices: vec![

        ]
    },
    Event {
        id: 48,
        name: "Green Meanies - Formation",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![

        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::Flag(Flag::IsFALC)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 49,
        name: "Success - Leisure",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::WorldVariable(WorldVariable::Outlook, 3.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Guaranteed,
                conditions: vec![
                    Condition::ResourceDemandGap(Resource::Labor, Comparator::LessEqual, -0.25)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 50,
        name: "Invasive Wines",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::WorldVariable(WorldVariable::Outlook, 1.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::ProjectStatus(2, ProjectStatus::Active)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 51,
        name: "Pollinator - Decline",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::Output(Output::PlantCalories, -5.0),
            Effect::TriggerEvent(52, 18)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::ProcessMixShareFeature(ProcessFeature::UsesPesticides, Comparator::GreaterEqual, 0.5)
                ]
            },
            Probability {
                likelihood: Likelihood::Guaranteed,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Biodiversity, Comparator::LessEqual, 0.3)
                ]
            },
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Biodiversity, Comparator::LessEqual, 0.5)
                ]
            },
            Probability {
                likelihood: Likelihood::Unlikely,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Biodiversity, Comparator::LessEqual, 0.7)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 52,
        name: "Pollinator - Collapse",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::Output(Output::PlantCalories, -10.0),
            Effect::Resource(Resource::Labor, -5.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::ProcessMixShareFeature(ProcessFeature::UsesPesticides, Comparator::GreaterEqual, 0.5)
                ]
            },
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Biodiversity, Comparator::LessEqual, 0.4)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 53,
        name: "Ocean Acidification",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![

        ],
        probabilities: vec![

        ],
        choices: vec![

        ]
    },
    Event {
        id: 54,
        name: "Ocean Microorganisms",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![

        ],
        probabilities: vec![

        ],
        choices: vec![

        ]
    },
    Event {
        id: 55,
        name: "Waste Leakage",
        locked: false,
        local: true,
        repeats: true,
        effects: vec![
            Effect::LocalVariable(LocalVariable::Health, -1.0),
            Effect::LocalVariable(LocalVariable::Outlook, -1.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::ProjectStatus(2, ProjectStatus::Inactive)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 56,
        name: "Flooded Farms",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::Output(Output::PlantCalories, -5.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Temperature, Comparator::GreaterEqual, 1.5)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 57,
        name: "SRM - Solar Impacts",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::OutputForFeature(ProcessFeature::IsSolar, -5.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::ProjectStatus(9, ProjectStatus::Active)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 58,
        name: "Black Market - Gasoline",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::WorldVariable(WorldVariable::Emissions, 1.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::OutputDemandGap(Output::Fuel, Comparator::Greater, 0.2)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 59,
        name: "Black Market - Meat",
        locked: false,
        local: true,
        repeats: true,
        effects: vec![
            Effect::WorldVariable(WorldVariable::Emissions, 1.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::OutputDemandGap(Output::AnimalCalories, Comparator::Greater, 0.25),
                    Condition::ProjectStatus(1, ProjectStatus::Active)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 60,
        name: "Coffee and Chocolate Decline",
        locked: false,
        local: true,
        repeats: true,
        effects: vec![
            Effect::WorldVariable(WorldVariable::Outlook, -1.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Temperature, Comparator::GreaterEqual, 1.5)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 61,
        name: "Sewage Overflow",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::LocalVariable(LocalVariable::Health, -1.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![

                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 62,
        name: "Amazon Savannah",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::WorldVariable(WorldVariable::Emissions, 2.0),
            Effect::WorldVariable(WorldVariable::Biodiversity, -2.0),
            Effect::WorldVariable(WorldVariable::Outlook, -1.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Likely,
                conditions: vec![
                    Condition::ProcessMixShareFeature(ProcessFeature::UsesLivestock, Comparator::GreaterEqual, 0.5)
                ]
            },
            Probability {
                likelihood: Likelihood::Likely,
                conditions: vec![
                    Condition::ProcessMixShare(0, Comparator::GreaterEqual, 0.5)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 63,
        name: "Locust Infestation",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::Output(Output::PlantCalories, -5.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Temperature, Comparator::GreaterEqual, 1.5)
                ]
            },
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Biodiversity, Comparator::LessEqual, 0.6)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 64,
        name: "Forest Infestation",
        locked: false,
        local: true,
        repeats: true,
        effects: vec![

        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Temperature, Comparator::GreaterEqual, 1.5)
                ]
            },
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Biodiversity, Comparator::LessEqual, 0.6)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 65,
        name: "CO2 Leak",
        locked: false,
        local: true,
        repeats: true,
        effects: vec![
            Effect::WorldVariable(WorldVariable::Emissions, 1.0),
            Effect::LocalVariable(LocalVariable::Health, -1.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::ProcessMixShareFeature(ProcessFeature::IsCCS, Comparator::GreaterEqual, 0.5)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 66,
        name: "SRM - Monsoon",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::Output(Output::PlantCalories, -5.0),
            Effect::LocalVariable(LocalVariable::Health, -1.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::ProjectStatus(9, ProjectStatus::Active)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 67,
        name: "Iron Fillings Dead Ocean",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::Demand(Output::PlantCalories, 10.0),
            Effect::WorldVariable(WorldVariable::Biodiversity, -1.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::ProjectStatus(13, ProjectStatus::Active)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 68,
        name: "West Antarctic Collapse",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::WorldVariable(WorldVariable::SeaLevelRise, 0.),
            Effect::TriggerEvent(72, 1)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Likely,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Temperature, Comparator::GreaterEqual, 2.5)
                ]
            },
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Temperature, Comparator::GreaterEqual, 2.0)
                ]
            },
            Probability {
                likelihood: Likelihood::Unlikely,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Temperature, Comparator::GreaterEqual, 1.5)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 69,
        name: "Polar Vortex",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::Output(Output::PlantCalories, -5.0),
            Effect::Output(Output::Electricity, -5.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Likely,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Temperature, Comparator::GreaterEqual, 2.0)
                ]
            },
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Temperature, Comparator::GreaterEqual, 1.5)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 70,
        name: "Mega Dust Storms",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::Resource(Resource::Labor, -5.0),
            Effect::LocalVariable(LocalVariable::Health, -1.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Likely,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Temperature, Comparator::GreaterEqual, 2.0)
                ]
            },
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Temperature, Comparator::GreaterEqual, 1.5)
                ]
            },
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::ProcessMixShareFeature(ProcessFeature::DegradesSoil, Comparator::GreaterEqual, 0.5)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 71,
        name: "Large Derecho Storm",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::LocalVariable(LocalVariable::Health, -1.0),
            Effect::Output(Output::PlantCalories, -5.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Likely,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Temperature, Comparator::GreaterEqual, 2.0)
                ]
            },
            Probability {
                likelihood: Likelihood::Random,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Temperature, Comparator::GreaterEqual, 1.5)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 72,
        name: "Ice Sheet Tsunami",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![

        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Rare,
                conditions: vec![

                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 73,
        name: "SRM - Assassination",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::WorldVariable(WorldVariable::Outlook, -2.0)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Unlikely,
                conditions: vec![
                    Condition::ProjectStatus(9, ProjectStatus::Active)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 74,
        name: "SRM - Respiratory Problems",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::WorldVariable(WorldVariable::Health, -2.0),
            Effect::AddEvent(73)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Unlikely,
                conditions: vec![
                    Condition::ProjectStatus(9, ProjectStatus::Active)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 75,
        name: "Greenland Collapse",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![
            Effect::WorldVariable(WorldVariable::SeaLevelRise, 0.),
            Effect::TriggerEvent(72, 1)
        ],
        probabilities: vec![

        ],
        choices: vec![

        ]
    },
    Event {
        id: 76,
        name: "Hot Soil Emissions",
        locked: true,
        local: true,
        repeats: true,
        effects: vec![

        ],
        probabilities: vec![

        ],
        choices: vec![

        ]
    },
    Event {
        id: 77,
        name: "First 100m-Inhabitant City",
        locked: false,
        local: true,
        repeats: true,
        effects: vec![

        ],
        probabilities: vec![

        ],
        choices: vec![

        ]
    },
    Event {
        id: 78,
        name: "Ex-Urban Explosion",
        locked: false,
        local: true,
        repeats: true,
        effects: vec![

        ],
        probabilities: vec![

        ],
        choices: vec![

        ]
    },
    Event {
        id: 79,
        name: "The Ark Proposal",
        locked: false,
        local: false,
        repeats: true,
        effects: vec![
            Effect::UnlocksProject(42)
        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Guaranteed,
                conditions: vec![
                    Condition::WorldVariable(WorldVariable::Biodiversity, Comparator::LessEqual, 0.1)
                ]
            }
        ],
        choices: vec![

        ]
    },
    Event {
        id: 80,
        name: "Gosplant Sentience",
        locked: false,
        local: true,
        repeats: true,
        effects: vec![

        ],
        probabilities: vec![
            Probability {
                likelihood: Likelihood::Improbable,
                conditions: vec![

                ]
            }
        ],
        choices: vec![

        ]
    }
];